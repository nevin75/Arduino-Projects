    ...
    class Part1Solution
    {
        static void Main(string[] args)
        {
            ...
            foreach (string instruction in directionArray)
            {
                ...
                if (instruction[0] == 'U')
                {
                    X += Int32.Parse(instruction.TrimStart(charsToTrim));
                    for (int i = xOld; (i <= X); i++)
                        map[i, Y] = true;
                }
                if (instruction[0] == 'D')
                {
            ...
            foreach (string secondInstruction in secondDirectionArray)
            {
                ...
                if (secondInstruction[0] == 'U')
                {
                    X += Int32.Parse(secondInstruction.TrimStart(charsToTrim));
                    for (int i = xOld; (i <= X); i++)
                        if ((map[i, Y] == true) && (i + Y < shortestRoute))
                            if ((map[i, Y] == true) && (Math.Abs(Y - centre) + Math.Abs(i - centre) < shortestRoute) && (Math.Abs(Y - centre) + Math.Abs(i - centre) > 0))
                            {
                                shortestRoute = Math.Abs(Y - centre) + Math.Abs(i - centre);
                                closestX = i - centre;
                                closestY = Y - centre;
                            }
                }
                if (secondInstruction[0] == 'D')
                {
                    X -= Int32.Parse(secondInstruction.TrimStart(charsToTrim));
                    for (int i = xOld; (i >= X); i--)
                        if ((map[i, Y] == true) && (Math.Abs(Y - centre) + Math.Abs(i - centre) < shortestRoute) && (Math.Abs(Y - centre) + Math.Abs(i - centre) > 0))
            ...
            Console.WriteLine($"The closest coordinates of intersection are {closestX}, {closestY}. The manhattan distance is {shortestRoute}.");
